...
#    get_xml()
    ...
    jsonstr = json.dumps(xpars, indent=1, ensure_ascii=False)  ##indent參數決定添加幾個空格
    ...
    for stop in stops:
        ...
        print(f'eqId = {stop["@eqId"]} freewayId={stop["@freewayId"]} url={videoUrl["@url"]}')
        value = (stop["@dbId"],
                 stop["@domainId"],
                 stop["@eqId"],
                 stop["@freewayId"],
...
# ###lambda處理
#     list_is_none = lambda x: x if x else ""
#     for stop in stops:
#         gifUrl = stop["gifUrl"]
#         print(f'{stop["@eqId"]} mile={stop["@freewayId"]} url={gifUrl["@url"]}')
#         value = (stop["@dbId"],
#                  stop["@domainId"],
#                  stop["@eqId"],
#                  stop["@freewayId"],
